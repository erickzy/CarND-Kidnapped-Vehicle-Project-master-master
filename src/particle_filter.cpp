...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
    ...
    // 10 particles, passed in 51.52s (.152, .142, .005)
    // 100 particles, passed in 52.76s (.115, .105, .004)
    // 1000 particles passed in 95.66s on my computer. Although this decreases the error, there is a significant performance drop. (.108, .101, .004)
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
    ...
    // "Return": The id of the observation landmarks will change to the corresponding id in the map (predicted)
